(function($) {
	...
	$.SenaTheme = {
		...
		//Page Loader
		loader:function() {
			if (SenaOptions.loader) {
				var loaderHide = function() {
					$(window).trigger('sena.loaded');
					
					$('.page-loader').delay(1000).fadeOut('slow', function() {
						$(window).trigger('sena.complete');
					});
				};
				...
				if ($('#intro').attr('data-type')==='video' && !Modernizr.touch) {
					$(window).on('sena.intro-video', function() {
		...
		navigation:function() {
			...
			$('.navbar .navbar-brand img').waitForImages(function() {
				try {
					...
					$('.navbar .navbar-nav > li > a, .navbar .navbar-icon').css('line-height', height+'px');
			...
			var floatingMenuShow = function() {
				...
				if (!$that.hasClass('process')) {
					...
					$old.animate({opacity:0}, {duration:75, queue:false});
			...
			if (SenaOptions.navigation==='sticky') {
				//Floating menu
			...
			$('a[href^=\\#]').on("click", function(e) {
				if ($(this).attr('href')!=='#' && !$(e.target).parent().parent().is('.navbar-nav') && !$(this).attr('data-toggle')) {
					$(document).scrollTo($(this).attr('href'), SenaOptions.scrollSpeed, {offset:{top:-85, left:0}});
					e.preventDefault();
				}
			});
			...
			$(document).ready(function() {
				$('.navbar-nav').onePageNav({
					currentClass:'active',
					changeHash:false,
					scrollSpeed:SenaOptions.scrollSpeed,
					scrollOffset:85,
					scrollThreshold:0.5,
					filter:'li a[href^=\\#]',
					begin:function() {
			...
			if (document.location.hash && SenaOptions.loader) {
		...
		intro:function() {
			...
			if ($intro.data('type')==='single-image') {
				...
				if ($elements.length>0) {
					...
					$(window).on('sena.complete', function() {
			...
			//Slideshow
				...
				var captionAnimation = function(e) {
					...
					$("h3").removeClass('animated '+animation);
					$("h1").removeClass('animated '+animation);
					$("p").removeClass('animated '+animation);
					$(".btn").removeClass('animated '+animation);
					
					$(".owl-item").not(".cloned").eq(item).find("h3").addClass('animated '+animation);
					$(".owl-item").not(".cloned").eq(item).find("h1").addClass('animated '+animation);
					$(".owl-item").not(".cloned").eq(item).find("p").addClass('animated '+animation);
					$(".owl-item").not(".cloned").eq(item).find(".btn").addClass('animated '+animation);
				};
				
				//Init carousel
				$('.intro .intro-slideshow').on('initialized.owl.carousel', function(e) {
					captionAnimation(e);
				});
                
                //Carousel
                $('.intro .intro-slideshow').owlCarousel({
                    items:1,
                    loop:true,
                    nav:true,
                    margin:0,
                    autoplay:true,
                    autoplayTimeout:$intro.data('delay')*1000,
                    smartSpeed:500,
					animateIn:'fadeIn',
                    animateOut:'fadeOut'
                });
				...
				//Change carousel
				$('.intro .intro-slideshow').on('changed.owl.carousel', function(e) {
					captionAnimation(e);
				});
			}
			//Fullscreen Video
				...
				$(document).ready(function() {
					...
					$(window).on('YTAPIReady', function() {
						reserveTimer = setTimeout(function() {
							$(window).trigger('sena.intro-video');
							onlyForFirst = false;
						}, 5000);
					});
		...
		portfolio:function() {
							...
							$container.waitForImages(function() {
								...
								$loading.animate({opacity:0}, {duration:200, queue:true, complete:function() {
									...
									that.imageSlider($container, function() {
										$(document).scrollTo($container, 600, {offset:{top:-parseInt($(".navbar").height(), 10), left:0}});
			...
			$('a[href^="#view-"]').on("click", function() {
				...
				if ($item.length>0) {
					$(document).scrollTo('#portfolio', SenaOptions.scrollSpeed, {offset:{top:-85, left:0}, onAfter:function() {
						$item.trigger('click');
					}});
				}
			});
		...
		mapCreate:function() {
			...
			if (SenaOptions.zoomControlDiv===null) {				
				var zoomControlDiv = document.createElement('div');		
				var zoomControl = new customZoomControl(zoomControlDiv, map);
				SenaOptions.zoomControlDiv = zoomControlDiv;
			}

			//Insert the zoom div on the top left of the map
			map.controls[google.maps.ControlPosition.LEFT_TOP].push(SenaOptions.zoomControlDiv);
		},
		...
		shortCodes:function() {
            ...
            if ($('.testimonial-slider').length>0) {
				...
				$(".testimonial-nav").slick({
					...
					focusOnSelect:true,
					variableWidth:false,
                    prevArrow:'<button type="button" class="slick-prev"><div class="icon icon-arrows-left"></div></button>',
                    nextArrow:'<button type="button" class="slick-next"><div class="icon icon-arrows-right"></div></button>',
					responsive:[
						{
							breakpoint:991,
							settings:{
...
function shareTo(network, title, image, url) {
	return $.SenaTheme.share(network, title, image, url);
}
